  ...
  return (
    <div className="stafi_header">
      ...
      <div className="info_span">
        {account == null &&
          (Tool.pageType() == rSymbol.Ksm ||
            Tool.pageType() == rSymbol.Dot) && (
            <div
        ...
        {account && (
          <div className="header_tools">
            <Popover
              ...
              onVisibleChange={setNoticePopoverVisible}
            >
              <div
                className={`notice_container ${pendingCount > 0 && "pending"}`}
              >
                <div
                  className={`header_tool notice ${
                    noticeData && noticeData.showNew && "new"
                  }`}
                >
                ...
                {pendingCount > 0 && (
                  <div className="notice_pending_text">
            ...
            {account.fisAccount && (
              ...
              >
                ...
                <div>
                  {StringUtil.replacePkh(account.fisAccount.address, 6, 44)}
                </div>
              </div>
            )}
            {account.dotAccount && (
              ...
              >
                ...
                <div>
                  {StringUtil.replacePkh(account.dotAccount.address, 6, 44)}
                </div>
              </div>
            )}
            {account.ksmAccount && (
              ...
              >
                ...
                <div>
                  {StringUtil.replacePkh(account.ksmAccount.address, 6, 44)}
                </div>
              </div>
            )}
            ...
            {account.type == "rATOM" &&
              (account.atomAccount ? (
                <div className="header_tool account">
                  ...
                  <div>
                    {StringUtil.replacePkh(account.atomAccount.address, 6, 38)}
                  </div>
                </div>
              ) : (
                <div
            ...
            {(account.type == "rSOL" || account.type == "rAsset") &&
              (account.solAccount ? (
                ...
                >
                  ...
                  <div>
                    {StringUtil.replacePkh(account.solAccount.address, 6, 38)}
                  </div>
                </div>
              ) : (
                <div
            ...
            {account.type !== "rBNB" && account.ethAccount && (
              <div style={{ display: "flex", flexDirection: "column" }}>
                <div className="header_tool account">
                  <div>
                    {account.ethAccount.balance || "--"}{" "}
                    {getMetaMaskTokenSymbol(metaMaskNetworkId)}
                  </div>
                  <div>
                    {StringUtil.replacePkh(account.ethAccount.address, 4, 38)}
                  </div>
                </div>
                {account.type === "rPool/lp" &&
                  ...
                  !liquidityPlatformMatchMetaMask(
                    metaMaskNetworkId,
                    getLpPlatformFromUrl(location.pathname)
            ...
            {account.bscAccount && (
              <div style={{ display: "flex", flexDirection: "column" }}>
                <div className="header_tool account">
                  ...
                  <div>
                    {StringUtil.replacePkh(account.bscAccount.address, 4, 38)}
                  </div>
                </div>
                {metaMaskNetworkId &&
                  !config.metaMaskNetworkIsBsc(metaMaskNetworkId) && (
                ...
                <div className="header_tool account">
                  <div>
                    {NumberUtil.handleFisAmountToFixed(
                      account.maticAccount.balance
                    )}{" "}
                    MATIC
                  </div>
                  <div>
                    {StringUtil.replacePkh(account.maticAccount.address, 4, 38)}
                  </div>
                </div>
                {metaMaskNetworkId &&
                  !config.metaMaskNetworkIsGoerliEth(metaMaskNetworkId) && (
