    ...
    ethereum
      ...
      .then((result: any) => {
        const balance = numberUtil.handleEthAmountToFixed(
          Web3Utils.fromWei(result, "ether")
        );
...
const getBep20Allowance = async (
  ...
  try {
    const allowance = await contract.methods
      .allowance(address, bridegServer.getBridgeBep20HandlerAddress())
      .call();
    cb && cb(allowance);
  } catch (e: any) {
    console.error(e);
...
export const clickSwapToBep20Link = (
