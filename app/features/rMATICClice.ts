...
export const unbond = (amount: string,recipient:string,willAmount:any, cb?: Function): AppThunk => async (dispatch, getState) => {
  try{
    ...
    dispatch(fisUnbond(amount, rSymbol.Matic, u8aToHex(keyringInstance.decodeAddress(recipient)), selectedPool.poolPubkey,"Unbond succeeded, unbonding period is around "+config.unboundAroundDays(Symbol.Matic)+" days", (r?:string) => {
      dispatch(reloadData()); 
      if(r == "Success"){  
...
export const onProceed = (txHash: string, cb?: Function): AppThunk => async (dispatch, getstate) => {
  ...
  if(result){ 
    ...
    dispatch(rTokenSeries_bondStates(rSymbol.Matic, bondSuccessParamArr, statusObj, (e: string) => {
      if(e=="successful"){ 
        dispatch(setStakeHash(null));
...
const add_Matic_stake_Notice=(uuid:string,amount:string,status:string,subData?:any):AppThunk=>async (dispatch,getState)=>{
  setTimeout(()=>{
...
const add_Matic_unbond_Notice=(uuid:string,amount:string,status:string,subData?:any):AppThunk=>async (dispatch,getState)=>{
  
  dispatch(add_Matic_Notice(uuid,noticeType.Staker,noticesubType.Unbond,amount,status,subData))
}
 
const add_Matic_Notice=(uuid:string,type:string,subType:string,content:string,status:string,subData?:any):AppThunk=>async (dispatch,getState)=>{
